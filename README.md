...
2. **`App.tsx`** renders the shell: workspace + right navigation rail. It owns `notesOpen` (whether the Notes panel is visible).
3. **`ContactDetailsPage`** fetches config data via `crmApi`, then wraps the workspace in **`CrmProvider`**.
4. **`CrmWorkspace`** renders the three main panels:
   - **Left** — Contact Details sidebar
   - **Center** — Conversations feed
...
|--------|----------------|--------|
| Select contact | `setSelectedContactId` | Updates sidebar, notes, conversations |
| Back arrow in sidebar | `setViewMode('list')` | Shows contact list |
...
**Contact list mode:** Clicking the back arrow sets `viewMode = 'list'`, which renders `ContactList` instead of the detail view. Selecting a contact switches back to `viewMode = 'detail'`.

**Add contact:** `ContactList` opens `AddContactModal`, which dynamically generates form fields from `contactFields.json`.
...
**Sending a message:** `ConversationComposer` calls `sendConversation(body, type)`, which appends an outbound email or chat to the active contact's conversation list.
